...
class VP8QualityFilter
{
    ...
    /**
     * externalSpatialLayerIdTarget and externalTemporalLayerIdTarget sync root.
     */
    private final Object externalTargetSyncRoot = new Object();

    /**
     * The target temporal layer ID that the *user* of this class wants to
     * achieve. Reading/writing of this field is synchronized on
     * {@link #externalTargetSyncRoot}.
     */
    private int externalTemporalLayerIdTarget = SUSPENDED_LAYER_ID;

    /**
     * The target spatial/quality layer ID that the *user* of this class wants
     * to achieve. Reading/writing of this field is synchronized on
     * {@link #externalTargetSyncRoot}.
     *
     * NOTE that the spatial layer id target that is instance tries to achieve
     * is stored in {@link #internalSpatialLayerIdTarget} and is updated in
     * the {@link #acceptFrame(RawPacket)} method.
     *
     * NOTE that the spatial layer id that is actually forwarded is stored by
     * {@link #currentSpatialLayerId} and is updated in the
     * {@link #acceptKeyframe(RawPacket)} method.
     */
    private int externalSpatialLayerIdTarget = SUSPENDED_LAYER_ID;

    /**
     * The spatial/quality layer id that this instance tries to achieve. Upon
     * receipt of a packet, we check whether {@link #externalSpatialLayerIdTarget}
     * is set to something different, in which case we set {@link #needsKeyframe}
     * equal to true and update . Reading/writing of this field is
     ...
     */
    synchronized boolean acceptFrame(@NotNull RawPacket firstPacketOfFrame)
    {
        ...
        // thread).
        int externalTemporalLayerIdTargetCopy, externalSpatialLayerIdTargetCopy;
        synchronized (externalTargetSyncRoot)
        {
            externalTemporalLayerIdTargetCopy = externalTemporalLayerIdTarget;
            externalSpatialLayerIdTargetCopy = externalSpatialLayerIdTarget;
        }

        if (externalSpatialLayerIdTargetCopy != internalSpatialLayerIdTarget)
        {
            // The externalSpatialLayerIdTarget has changed since accept last
            // run; perhaps we should request a keyframe.
            internalSpatialLayerIdTarget = externalSpatialLayerIdTargetCopy;
            if (externalSpatialLayerIdTargetCopy > SUSPENDED_LAYER_ID)
            {
                needsKeyframe = true;
            }
        }

        if (externalSpatialLayerIdTargetCopy < 0
            || externalTemporalLayerIdTargetCopy < 0)
        {
        ...
        else if (currentSpatialLayerId > SUSPENDED_LAYER_ID)
        {
            ...
            if (currentSpatialLayerId > externalSpatialLayerIdTargetCopy)
            {
                // pending downscale, decrease the frame rate until we
                // downscale.
                return temporalLayerIdOfFrame < 1;
            }
            else if (currentSpatialLayerId < externalSpatialLayerIdTargetCopy)
            {
            ...
            else
            {
                ...
                return temporalLayerIdOfFrame <= externalTemporalLayerIdTargetCopy;
            }
        }
        else
        {
    ...
    /**
     * Sets the target quality for this quality filter.
     *
     * @param value the target quality for this quality filter.
     */
    public void setTarget(int value)
    {
        synchronized (externalTargetSyncRoot)
        {
            if (value > -1)
            {
                externalTemporalLayerIdTarget = value % 3;
                externalSpatialLayerIdTarget = value / 3;
            }
            else
            {
                externalTemporalLayerIdTarget = -1;
                externalSpatialLayerIdTarget = -1;
            }
        }
    }

    /**
     * @return the target quality for this quality filter.
     */
    public int getTarget()
    {
        synchronized (externalTargetSyncRoot)
        {
            if (externalTemporalLayerIdTarget < 0
                || externalSpatialLayerIdTarget < 0)
            {
                return -1;
            }
            else
            {
                return externalTemporalLayerIdTarget
                    + externalSpatialLayerIdTarget * 3;
            }
        }
    }
}
